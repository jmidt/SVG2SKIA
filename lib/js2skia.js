...
module.exports = function(svgjs) {
  ...
  var parse_root = function(attrs) {
    ...
    console.log('SkFont font = {SkTypeface::MakeDefault(), 20};');
  };
  ...
  var parse_text = function(node) {

  }

  var parse_tspan = function(node) {
    const x = node.attrs['x'].value
    const y = node.attrs['y'].value
    /// TODO: probably not general enough
    const str = node.content[0]['text'].value
    const params = [x, y, str].join(', ')
    console.log('ctx.drawString("' + params + ', font, paint);')
  }
  ...
  var visit_node = function(node, depth) {
    var parse_styles = function(attrs) {
      for (const attrkey in attrs) {
        ...
        switch (attr.name) {
          ...
          case 'font-size': {
            console.log('font.setFontsize(' + attr.value + ');')
            break;
          }
          case 'font-weight': {
            /// TODO, this could be done better, depending on what fonts are available
            if (attr.value == 'normal') {console.log('font.setEmbolden(false);'); break;}
            if (attr.value == 'bold') {console.log('font.setEmbolden(true);'); break;}
            const val = parseFloat(attr.value)
            if (val < 501) console.log('font.setEmbolden(false);')
            else console.log('font.setEmbolden(true);')
            break;
          }
          // Recursively unpack 'style' attributes
          case 'style': {
            parse_styles(attr);
            break;
          }
          // List of purposely ignored styles: line-height, font-family
        }
      }
    };
    ...
    switch (node.elem) {
      ...
      case 'path':
        ...
        break;
      case 'text':
        parse_text(node.attrs);
        break;
      case 'tspan':
        parse_tspan(node);
        
      case 'defs':
        // Don't call children. Defs are not supported so far.
        return;
      default:
